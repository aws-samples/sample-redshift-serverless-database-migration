	...
	for y in $(seq 1 "${param_count}"); do
		if [ "${y}" -eq "1" ]; then
			...
			SELECT CASE 
				...
				COALESCE(p.proargnames[${y}], '') || ' ' || COALESCE(t.typname, split_part(oidvectortypes(p.proargtypes), ',', ${y})) 
			FROM pg_proc_info p 
			LEFT JOIN pg_type t ON t.oid = p.proallargtypes[${y}] WHERE p.prooid = ${oid}" 2>/dev/null)
			params+=", ${param}"
		fi
	done
	params+=")"
...
alter_schema_owner()
{
	...
	WHERE database_name = current_database() AND schema_type = 'local' AND schema_name IN ${SCHEMAS} AND pg_get_userbyid(schema_owner) <> 'rdsdb'")
	...
	FROM svv_all_schemas 
	WHERE database_name = current_database() AND schema_type = 'local' AND schema_name IN ${SCHEMAS} AND pg_get_userbyid(schema_owner) <> 'rdsdb'"); do
		i=$((i+1))
		exec_sql="${exec_dir}/${prefix}_${i}.sql"
		schema_name=$(echo ${x} | awk -F '|' '{print $1}')
		schema_owner=$TARGET_PGUSER
...
alter_table_owner()
{
	...
	WHERE s.database_name = current_database() AND s.schema_type = 'local' AND s.schema_name IN ${SCHEMAS} AND relkind = 'r' AND c.relname NOT LIKE 'mv_tbl__%' AND a.min_attsortkeyord >= 0")
	echo "INFO: ${prefix}:creating ${obj_count}"
	for schema_name in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "
	SELECT schema_name 
	FROM svv_all_schemas 
	WHERE database_name = current_database() AND schema_type = 'local' AND schema_name IN ${SCHEMAS}
...
alter_function_owner()
{
	...
	JOIN svv_all_schemas s ON s.schema_name = n.nspname 
	WHERE s.database_name = current_database() AND s.schema_type = 'local' AND s.schema_name IN ${SCHEMAS} AND l.lanname IN ('sql', 'plpythonu') AND u.usename <> 'rdsdb' AND p.prokind = 'f'")
	echo "INFO: ${prefix}:creating ${obj_count}"
	for schema_name in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "
	SELECT schema_name 
	FROM svv_all_schemas 
...
alter_procedure_owner()
{
	...
	JOIN pg_user u ON p.proowner = u.usesysid 
	JOIN svv_all_schemas s ON s.schema_name = n.nspname 
	WHERE s.database_name = current_database() AND s.schema_type = 'local' AND s.schema_name IN ${SCHEMAS} AND l.lanname = 'plpgsql' AND p.proname NOT LIKE 'mv_sp__%' AND u.usename <> 'rdsdb' AND p.prokind = 'p'")
	echo "INFO: ${prefix}:creating ${obj_count}"
	for schema_name in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "
	SELECT schema_name 
	FROM svv_all_schemas 
...
grant_user_schema()
{
	...
	JOIN pg_user u ON u.usename = split_part(sub.acl, '=', 1) WHERE u.usename <> 'rdsdb' AND u.usesuper IS FALSE AND sub.nspname IN ${SCHEMAS}")
	echo "INFO: ${prefix}:creating ${obj_count}"
	for x in $(psql -h $SOURCE_PGHOST -p $SOURCE_PGPORT -d $SOURCE_PGDATABASE -U $SOURCE_PGUSER -t -A -c "
	...
	WHERE u.usename <> 'rdsdb' AND u.usesuper IS FALSE AND sub.nspname IN ${SCHEMAS}
	...
	ORDER BY schema_name"); do
		...
		if [ "${public_check}" -eq "0" ]; then
			...
			FROM 	(
				...
				WHERE u.usename <> 'rdsdb' AND u.usesuper IS FALSE AND sub2.nspname IN ${SCHEMAS}
				) AS sub3
...
grant_default_schema_user()
{
	...
	WHERE sub2.nspname IN ${SCHEMAS} ORDER BY 2, 3, 4"); do
...
DEFAULTS_FAILED=0
if [ -d "$PWD/log" ]; then
    OWNERS_FAILED=$(grep -rl "ERROR:" $PWD/log/alter_*_owner_*.log 2>/dev/null | wc -l)
    GRANTS_FAILED=$(grep -rl "ERROR:" $PWD/log/grant_*_*.log 2>/dev/null | wc -l)
    DEFAULTS_FAILED=$(grep -rl "ERROR:" $PWD/log/grant_default_*.log 2>/dev/null | wc -l)
fi

## Summary
echo "============================================"
echo "INFO: Migration Summary - Permissions"
echo "============================================"
echo "  Owner changes failed:   ${OWNERS_FAILED}"
echo "  Grant operations failed: ${GRANTS_FAILED}"
echo "  Default grants failed:   ${DEFAULTS_FAILED}"
echo "============================================"

TOTAL_FAILED=$((OWNERS_FAILED + GRANTS_FAILED + DEFAULTS_FAILED))
if [ $TOTAL_FAILED -gt 0 ]; then
    echo "WARN: ${TOTAL_FAILED} permission operations failed. Check logs in $PWD/log/ for details."
fi
